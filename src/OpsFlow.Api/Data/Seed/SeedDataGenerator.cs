...
public static class SeedDataGenerator
{
    ...
    private static readonly (string Email, string DisplayName, UserRole Role)[] UserSeeds =
    [
        ("admin@opsflow.local", "Admin User", UserRole.Admin),
        ("manager@opsflow.local", "Morgan Manager", UserRole.Manager),
        ("analyst1@opsflow.local", "Alex Analyst", UserRole.Analyst),
        ("analyst2@opsflow.local", "Blair Analyst", UserRole.Analyst),
        ("analyst3@opsflow.local", "Casey Analyst", UserRole.Analyst)
    ];
    ...
    public static SeedDataSet Generate(DateTime nowUtc)
    {
        ...
        var analysts = users.Where(x => x.Role == UserRole.Analyst).ToArray();
        ...
        for (var index = 1; index <= CaseCount; index++)
        {
            ...
            var resolvedAtUtc = status is CaseStatus.Resolved or CaseStatus.Closed
                ? createdAtUtc.AddHours(Math.Min(targetHours + 4, 96))
                : (DateTime?)null;
            var closedAtUtc = status == CaseStatus.Closed
                ? resolvedAtUtc?.AddHours(8)
                : null;
            var requiresApproval = priority is CasePriority.Critical or CasePriority.High;
